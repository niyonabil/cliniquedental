...
const getFilePath = (entity: string) => path.join(DATA_DIR, `${entity}.dat`);
...
app.delete('/api/patients/:id', (req, res) => {
    try {
        ...
        const filteredPatients = patients.filter((p: any) => p.id !== req.params.id);
...
app.put('/api/appointments/:id', (req, res) => {
    try {
        ...
        const index = appointments.findIndex((a: any) => a.id === req.params.id);
        if (index === -1) {
...
app.delete('/api/appointments/:id', (req, res) => {
    try {
        ...
        const filteredAppointments = appointments.filter((a: any) => a.id !== req.params.id);
